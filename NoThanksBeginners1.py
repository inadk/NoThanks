...
class MyAI(Player):
    ...
    def __init__( self ):
        ...
        # collect statistics 
        self.statistics = {
            'take_counts': {coins: 0 for coins in range(70 + 1)},
            'take_rates': {coins: [] for coins in range(70 + 1)}
        }
...
decision = take( self, card, state )

 def make_move(self, card, state):
    current_coins = self.coins  
    if decision == 'take':
        self.statistics['take_counts'][current_coins] += 1
        ...
